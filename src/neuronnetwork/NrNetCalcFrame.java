...
public class NrNetCalcFrame extends JFrame {
  ...
  private void jbInit() throws Exception {
    ...
    jTable1.setPreferredScrollableViewportSize(new Dimension(columnNames.length*45, 70));
  ...
  void jButton1_actionPerformed(ActionEvent e) {
    ...
    try {
      ...
      for(i=0;i<tblData.length;i++) {
        nrn.netReset();
        for(j=0;j<iQntOtp;j++)
          iOtNrNet[i][j][1]=Float.valueOf(tblData[i][j].toString()).floatValue();
...
//    iY0=jPanel2.getHeight()/2;
    ...
    for(i=0;i<iOtNrNet[0].length;i++){
      ...
      for(j=1;j<iOtNrNet.length;j++) {
        gPnl.drawLine(10+iXStep*j, iYCoordLine, 10+iXStep*j,iYCoordLine+iYCoordLineSteep+iYCoordLineSteep);
      }
