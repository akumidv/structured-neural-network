...
/*    jCmBxModel.addInputMethodListener(new java.awt.event.InputMethodListener() {
  ...
  public void jMenuFileOpen_actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            String strLine;
            strLine=in.readLine();
            if(!(strLine.startsWith("#NeuralNetwok network data file") || strLine.startsWith("#NeuralNetwok network teach analyse net temp data file"))) {System.err.println("Invalid network data file: " + filename);in.close();return; }
            do {strLine=in.readLine();} while(strLine.startsWith("#"));
            if (!strLine.startsWith("[Calc model]")) { System.err.println("Invalid network data file: " + filename); in.close(); return; }
            do {strLine=in.readLine();} while(strLine.startsWith("#"));
            int iCalcMd=Integer.valueOf(strLine).intValue();
            do {strLine=in.readLine();} while(strLine.startsWith("#"));
            if (!strLine.startsWith("[Teach model]")) { System.err.println("Invalid network data file: " + filename); in.close(); return; }
            do {strLine=in.readLine();} while(strLine.startsWith("#"));
            int iTeachMd=Integer.valueOf(strLine).intValue();
            do {strLine=in.readLine();} while(strLine.startsWith("#"));
            if (!strLine.startsWith("[Quantity layers]")) { System.err.println("Invalid network data file: " + filename); in.close(); return; }
            do {strLine=in.readLine();} while(strLine.startsWith("#"));
            int iQntLy=Integer.valueOf(strLine).intValue();
            nrn.CreateNrNet(iQntLy,iCalcMd,iTeachMd);
            for(int i=0;i<iQntLy;i++) {
              do {strLine=in.readLine();} while(strLine.startsWith("#"));
              if (!strLine.startsWith("[Quantity Neurons In Layer: "+i+"]")) { System.err.println("Invalid network data file: " + filename); in.close(); return; }
              do {strLine=in.readLine();} while(strLine.startsWith("#"));
              int iQntLyNr=Integer.valueOf(strLine).intValue();
              nrn.CreateLyNrNet(i,iQntLyNr);
              for(int j=0;j<iQntLyNr;j++) {
                do {strLine=in.readLine();} while(strLine.startsWith("#"));
                if (!strLine.startsWith("[Neuron "+i+":"+j+"]")) { System.err.println("Invalid network data file: " + filename); in.close(); return; }
                /*Параметры нейрона*/
                do {strLine=in.readLine();} while(strLine.startsWith("#"));
                holdstr = strLine;
